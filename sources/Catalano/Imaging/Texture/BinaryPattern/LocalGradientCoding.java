...
public class LocalGradientCoding implements IBinaryPattern {
    ...
    public ImageHistogram ProcessImage(FastBitmap fastBitmap) {
        if (!fastBitmap.isGrayscale()) {
            throw new IllegalArgumentException("LGC works only with grayscale images.");
        }
        ...
        for (int i = 1; i < height; i++) {
            for (int i2 = 1; i2 < width; i2++) {
                int i3 = fastBitmap.getGray(i + (-1), i2 + (-1)) - fastBitmap.getGray(i + (-1), i2 + 1) >= 0 ? 128 : 0;
                ...
                if (fastBitmap.getGray(i, i2 - 1) - fastBitmap.getGray(i + 1, i2) >= 0) {
                    i3 += 8;
                }
                if (fastBitmap.getGray(i - 1, i2 + 1) - fastBitmap.getGray(i + 1, i2 + 1) >= 0) {
                    i3 += 4;
